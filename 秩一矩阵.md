...
> 5. 谱分解，对于一个 n 阶实对称矩阵 A，如果 $\lambda_{1},\lambda_{2},\dots,\lambda n$  是它的特征值，而 $\mathbf{u}_{1},\mathbf{u}_{2},\dots,\mathbf{u}_{n}$是对应的一组标准正交的特征向量（即它们两两正交，且每个向量的长度都为1），则A可以表示为：$A = \lambda_1 \mathbf{u}_1 \mathbf{u}_1^T + \lambda_2 \mathbf{u}_2 \mathbf{u}_2^T + \cdots + \lambda_n \mathbf{u}_n \mathbf{u}_n^T$ 
> 	1. 对于秩一矩阵，即 $A = tr(A) \mathbf{u}\mathbf{u}^{T}$
