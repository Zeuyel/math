...
> [!abstract] 分块矩阵
> 1. [[线性代数字典#广义初等变换]]。 
> 2. 分块矩阵的地位是工具，一方面主动考察其运算（广义初等变换）。另一方面，要把握住其对于矩阵之间运算转换的简便表达，他是研究秩、计算行列式的好方法，我们要主动地去思考使用分块矩阵化简，就像遇到了多项式想到因式分解一般。特别常见于 [[抽象行列式]]
...
C.$r \left(  \begin{matrix} BA \\ B^TBA \end{matrix} \right)=r(AB^T)$.
...
$$
D.$r \left(\begin{matrix} A^TA \\ B^TA \end{matrix} \right)=r(A)$.
