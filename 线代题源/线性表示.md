...
>		2. $r(A) = r(\bar{A}) = n$
>		3. $r(A) = r(\bar{A}) < n$  
>4. 线性表示的分析。一般而言，必要性上，我们从被表示的向量组出发，分析秩的情况。而充分性，就需要进一步分析空间，往往借助于满秩来得出。
>	1. 必要性上 $A = BC$ 那么 $r(B) \ge r(A)$ 
>	2. 充分性上就比较难了，因为向量组等价还要说明空间的相等
>		1. 如果 $A,B$ 是 三行的矩阵，又有 $A = BC$ 同时 $r(A)= 3$ 就可以得到 $B = AC$    
>		2. 或者 $A = BC$ 中分析出 $C$ 是行满秩的。  
>5. 线性表示等价于两次正交，$\gamma_{1}$ 和 $\alpha_{1},\alpha_{2}$   正交，而 $\alpha_{3}$ 和 $\gamma_{1}$ 正交 $\implies$ $\alpha_{1},\alpha_{2}$ 可以线性表示 $\alpha_{3}$     
...
设向量$a_1 = \begin{pmatrix} a \\ 1 \\ -1 \\ 1 \end{pmatrix}$ , $a_2 = \begin{pmatrix} 1 \\ 1 \\ b \\ a \end{pmatrix}$ , $a_3 = \begin{pmatrix} 1 \\ a  \\ -1 \\ 1 \end{pmatrix}$ 。若$a_1$,$a_2$,$a_3$线性相关，且其中任意两个向量线性无关，则( )
...
按照惯性，我们得到，向量组的秩为二，又通过列秩和行秩相等得到了下列矩阵行秩为二：
...
那么，非常有可能直接选择 $a = 1,b\neq-1$ ，然而这只能保证秩为二，而秩为二对应了多种向量关系，题目中给出的空间关系为任意两个向量均线性无关，显然当 $a= 1$ $a_{1} = a_{3}$ 故而舍弃。
