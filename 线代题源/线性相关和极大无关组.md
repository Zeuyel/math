...
> 	1. 线性方程组仅有0解
> 	2. 解出 k 是 0
> 3. 使用秩作为工具研究，要善于使用其不等式传导。有两种思路：
> 	1. 双向的夹逼
> 	2. 单项的不等号最后回到原式值，则其中的所有不等号都要取等号
...
事实上判断出秩是简单的，我们说明回到定义的证明方法：

已知向量组 $(I) a_1, a_2, a_3$; $(II) a_1, a_2, a_3, a_4$; $(III) a_1, a_2, a_3, a_5$. 若向量组 $(I)$, $(II)$, $(III)$ 的秩分别为 $3,3,4$, 证明：向量组 $a_1, a_2, a_3, a_5 - a_4$ 的秩为 4.

设 $x_1a_1 + x_2a_2 + x_3a_3 + x_4(a_5 - a_4) = 0$ 并且存在 $y_{1},y_{2},y_{3}$ 使得：$a_{4} = y_{1}a_{1} + y_{2}a_{2} + y_{3}a_{3}$

$$(x_1 - y_1x_4)a_1 + (x_2 - y_2x_4)a_2 + (x_3 - y_3x_4)a_3 + x_4a_5 = 0$$

由 $a_1, a_2, a_3, a_5$线性无关， 

$$
\begin{cases} x_1 - y_1x_4 = 0, \\ x_2 - y_2x_4 = 0, \\ x_3 - y_3x_4 = 0, \\ x_4 = 0, \end{cases}
$$

故而有$x_{1} = x_{2} = x_{3} = x_{4} =0$ 即 $a_{1},a_{2},a_{3},a_{5} - a_{4}$ 线性无关。
